const Img = ({url, caption}) => {
    return (
        <div>
            <img src={url} alt="" />
            { caption.length ? <p className="w-full text-center my-3 md_mb-12 text-base text-dark-grey">{caption}</p> : "" }
        </div>
    )
...
const Quote = ({quote, caption}) => {
    return (
        <div className="bg-purple/10 p-3 pl-5 border-l-4 border-x-purple">
...
const BlogContent = ({block}) => {
    ...
    if(type == "list") {
        return <List stlye={data.style} items={data.items} />
    }
}
